    ...
    fun fetchDolarRate() {
        viewModelScope.launch {
            ...
            val rate = withContext(Dispatchers.IO) {
                try {
                    // Usamos ve.dolarapi.com (el espejo) y aumentamos el tiempo de espera
                    ...
                    for (i in 0 until jsonArray.length()) {
                        ...
                        if (obj.getString("nombre").contains("BCV", ignoreCase = true)) {
                            rateFound = obj.getDouble("promedio")
                            break
                        }
                    }
                    rateFound
                } catch (e: Exception) {
                    40.0 // Si falla, usa el respaldo
                }
            }
